...
public class ProducerOfertaPaqueteRest {
	...
	public Response buscarOfertaPaquete(@PathParam("idPaquete") int idPaquete,
			...
			@PathParam("cupo") int cupo, @PathParam("cantPersonas") int cantPersonas,
			@PathParam("idAgencia") String idAgencia, @PathParam("nombreAgencia") String nombreAgencia,
			@PathParam("direccionAgencia") String direccionAgencia, @PathParam("estadoAgencia") String estadoAgencia,
			@PathParam("foto") String foto, @PathParam("fechaDesde") String fechaDesde,
			@PathParam("fechaHasta") String fechaHasta, @PathParam("estado") String estado,
			@PathParam("precio") float precio, @PathParam("descripcion") String descripcion,
			@PathParam("politicaCancelacion") String politicaCancelacion, @PathParam("servicios") String servicios,
			@PathParam("mediosDePago") String mediosDePago) {
		try {
			testingQueue.sendMessage(idPaquete, nombrePaquete, idCiudadDestino, cupo, cantPersonas, idAgencia,
					nombreAgencia, direccionAgencia, estadoAgencia, foto, fechaDesde, fechaHasta, estado, precio,
					descripcion, politicaCancelacion, servicios, mediosDePago);
